...
test('Non-stocked items disappear when checkbox is checked', () => {
  const products = [
      {category: "Sporting Goods", price: "$49.99", stocked: true, name: "Football"},
      {category: "Sporting Goods", price: "$9.99", stocked: true, name: "Baseball"},
      {category: "Sporting Goods", price: "$29.99", stocked: false, name: "Basketball"},
  ...
  /*
      ...
      then send a click event to the checkbox to filter out non-stocked items
  */
  const filterCheckboxInput = getByLabelText("Only show products in stock");
  expect(filterCheckboxInput.checked).toBeFalsy();
...
/*
    A parameterized test to cover the following cases
    1. No filter text
    2. filter text that results in some non-stocked products which should not be displayed due to filter checkbox begin checked
